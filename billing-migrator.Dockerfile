...
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o billing-migrator ./cmd/billing-migrator
...
RUN chown -R 1001:0 /app && \
    chmod -R g=u /app && \
    chmod +x /usr/local/bin/billing-migrator

# Switch to non-root user
USER 1001

# Default command runs migrations
CMD ["billing-migrator", "up"]